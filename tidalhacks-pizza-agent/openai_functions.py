        ...
        try:
            response = await asyncio.wait_for(
                openai_client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are an intent classifier for a pizza coupon agent at TidalHACKS25. Respond only with valid JSON."},
...
async def openai_generate_unique_prompt(retry_count: int = None) -> str:
    """
    Generate a unique, creative prompt using OpenAI for TidalHACKS25 with fallback
    """
        ...
        try:
            response = await asyncio.wait_for(
                openai_client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are a creative prompt generator for a pizza coupon bot at TidalHACKS25. Generate ONE single fun prompt that celebrates hackers and pizza. Do not create multiple examples or numbered prompts."},
                        {"role": "user", "content": prompt}
                    ],
        ...
        try:
            response = await asyncio.wait_for(
                openai_client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are a fun, enthusiastic pizza agent at TidalHACKS25 powered by Fetch.ai. You celebrate hackers' stories and give personalized responses."},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.8,
                    max_tokens=250
